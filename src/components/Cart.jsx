...
// console.log(productsWithDetails);
  ...
  const deleteAll = async()=>{
    try {
     const response = await axios.post(`http://localhost:8080/delete_all/${userId}`)
  ...
  try {
    ...
    // 서버에 삭제 요청 전송
    await axios.post(`http://localhost:8080/delete_checked_items/${userId}`, { items: itemsToDelete });

    // 삭제 후 장바구니 데이터 가져오기
    const updatedCartResponse = await axios.get(`http://localhost:8080/get_cart/${userId}`);
